...
interface Props {
  logic: ReturnType<typeof useHolidayManager>;
}

export function HolidayAssignModal({ logic }: Props) {
  const {
    isAssignOpen,
    setIsAssignOpen,
    selectedTarget,
      ...
      <DialogContent className="!w-[60vw] !max-w-[95vw] max-h-[85vh] flex flex-col bg-gray-900 border-gray-700 text-gray-100">
        <DialogHeader>
          <DialogTitle className="text-white">
            Atur Pegawai Libur: {selectedTarget?.description}
          </DialogTitle>
          <DialogDescription className="text-gray-400">
            Pilih pegawai yang berhak mendapatkan libur pada rentang tanggal{" "}
            {selectedTarget?.startDate || selectedTarget?.date} s/d{" "}
            {selectedTarget?.endDate || selectedTarget?.date}. Kosongkan pilihan
            jika libur berlaku untuk semua pegawai.
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="flex flex-col sm:flex-row items-center gap-4 mt-2">
          ...
          <Select
            value={divisiFilter}
            onValueChange={(v) => setDivisiFilter(v ?? "all")}
          >
        ...
        <DialogFooter className="border-t border-gray-800 pt-4 mt-2">
          <span className="text-sm font-medium mr-auto text-emerald-400 mt-2">
            Pegawai Dikecualikan/Ditugaskan Spesifik: {selectedUserIds.length}{" "}
            Orang
          </span>
          <Button
            variant="outline"
