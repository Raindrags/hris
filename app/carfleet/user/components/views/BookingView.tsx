...
import { Car, Bus, Check, ClipboardList, ArrowRight, Info, ChevronLeft, ChevronRight } from 'lucide-react';

interface BookingViewProps {
  selectedFleet: number;
  setSelectedFleet: (id: number) => void;
  selectedDate: string; // Menggunakan format "YYYY-MM-DD" agar mencakup bulan & tahun
  setSelectedDate: (date: string) => void;
  openModal: () => void;
}

export default function BookingView({ selectedFleet, setSelectedFleet, selectedDate, setSelectedDate, openModal }: BookingViewProps) {
  ...
  return (
    <div className="w-full transition-all duration-300">
      ...
      {/* HEADER */}
      <div className="mb-6">
        <h3 className="text-2xl font-black text-[#1a365d]">Sewa Mandiri Kendaraan</h3>
        <p className="text-slate-500 text-sm">Gunakan form ini jika Anda memerlukan satu armada penuh untuk kegiatan dinas atau rombongan khusus.</p>
      </div>
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            >
              ...
              <h5 className="font-extrabold text-[#1a365d] text-base">Toyota Hiace Commuter</h5>
              <p className="text-xs text-slate-400 font-bold mt-1 mb-3">Plat B 1234 SCH • 15 Kursi</p>
              <span className="inline-block bg-teal-50 text-[#0d9488] text-[10px] font-extrabold px-3 py-1 rounded-full uppercase tracking-wider">Pilihan Utama Rombongan</span>
            </div>
            ...
            >
              ...
              <h5 className="font-extrabold text-[#1a365d] text-base">Kijang Innova Reborn</h5>
              <p className="text-xs text-slate-400 font-bold mt-1 mb-3">Plat B 5678 SCH • 7 Kursi</p>
              <span className="inline-block bg-slate-100 text-slate-500 text-[10px] font-extrabold px-3 py-1 rounded-full uppercase tracking-wider">Perjalanan Dinas Standard</span>
            </div>
          </div>
             ...
             <div className="flex justify-between items-center mb-6">
                <div className="flex items-center gap-3">
                  <h5 className="font-extrabold text-[#1a365d] text-base">
                    Ketersediaan Kalender ({monthNames[currentMonth]} {currentYear})
                  </h5>
                  <div className="flex gap-1 bg-slate-100 p-1 rounded-xl">
                    <button onClick={handlePrevMonth} className="p-1 hover:bg-white rounded-lg text-slate-600 transition">
                      <ChevronLeft className="w-4 h-4" />
                    </button>
                    <button onClick={handleNextMonth} className="p-1 hover:bg-white rounded-lg text-slate-600 transition">
                      <ChevronRight className="w-4 h-4" />
                    </button>
                  </div>
                </div>
                <span className="text-xs text-slate-400 font-bold hidden sm:inline">Tekan tanggal untuk memilih</span>
             </div>
          ...
          <div className="bg-white border border-slate-100 rounded-3xl p-6 shadow-sm space-y-4">
             <div className="border-b border-slate-100 pb-4">
                ...
                <h5 className="font-extrabold text-[#1a365d] mt-0.5">
                  {selectedFleet === 1 ? 'Toyota Hiace Commuter' : 'Kijang Innova Reborn'}
                </h5>
             </div>
             ...
             <div className="bg-teal-50 border border-teal-100 rounded-2xl p-4 flex items-start gap-3 text-[#0d9488]">
                ...
                <p className="text-xs font-semibold leading-relaxed">Kendaraan ini gratis digunakan untuk operasional resmi sekolah setelah disetujui GA.</p>
             </div>
