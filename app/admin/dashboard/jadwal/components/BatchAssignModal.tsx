...
export function BatchAssignModal({ assignLogic }: { assignLogic: ReturnType<typeof useBatchAssign> }) {
  const {
    isOpen, setIsOpen, selectedShift, divisions, selectedUserIds,
    searchTerm, setSearchTerm, divisiFilter, setDivisiFilter,
    currentPage, setCurrentPage, totalPages,
    filteredEmployees, paginatedEmployees,
    handleSave, toggleEmployee, toggleAll
  } = assignLogic;
  ...
  return (
    <Dialog open={isOpen} onOpenChange={setIsOpen}>
      <DialogContent className="!w-[60vw] !max-w-[95vw] max-h-[85vh] flex flex-col bg-gray-900 border-gray-700 text-gray-100">
        <DialogHeader>
          <DialogTitle className="text-white">Atur Pegawai untuk: {selectedShift?.name}</DialogTitle>
          <DialogDescription className="text-gray-400">Pilih pegawai yang akan ditugaskan ke template jadwal ini.</DialogDescription>
        </DialogHeader>
        ...
        <div className="flex flex-col sm:flex-row items-center gap-4 mt-2">
          ...
          <Select value={divisiFilter} onValueChange={(value) => setDivisiFilter(value ?? "all")}>
                ...
                <TableRow>
                  <TableHead className="w-12 text-center">
                    <Checkbox
                      ...
                      onCheckedChange={(c) => toggleAll(!!c, filteredEmployees.map((e) => e.id))}
                    />
                  </TableHead>
          ...
          {totalPages > 1 && (
            <div className="flex items-center justify-between px-4 py-2 bg-gray-800/30 border-t border-gray-800">
              ...
              <div className="flex items-center gap-2">
                <Button variant="outline" size="sm" onClick={() => setCurrentPage((p) => Math.max(1, p - 1))} disabled={currentPage === 1} className="border-gray-700 bg-gray-800 text-gray-200 hover:bg-gray-700 hover:text-white disabled:opacity-50">
                  <ChevronLeft className="w-4 h-4 mr-1" /> Prev
                </Button>
                <span className="text-sm font-medium px-2 text-gray-300">Hal {currentPage} / {totalPages}</span>
                <Button variant="outline" size="sm" onClick={() => setCurrentPage((p) => Math.min(totalPages, p + 1))} disabled={currentPage === totalPages} className="border-gray-700 bg-gray-800 text-gray-200 hover:bg-gray-700 hover:text-white disabled:opacity-50">
        ...
        <DialogFooter className="border-t border-gray-800 pt-4 mt-2">
          <span className="text-sm font-medium mr-auto text-blue-400 mt-2">Total Dipilih: {selectedUserIds.length} Pegawai</span>
          <Button variant="outline" onClick={() => setIsOpen(false)} className="border-gray-700 text-gray-300 hover:bg-gray-800 hover:text-white">Batal</Button>
          <Button onClick={handleSave} className="bg-crimson-700 hover:bg-crimson-800 text-white">Simpan Penugasan</Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
  );
