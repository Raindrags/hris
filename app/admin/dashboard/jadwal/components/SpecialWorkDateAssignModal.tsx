...
interface Props {
  assignLogic: ReturnType<typeof useSpecialWorkDateAssign>;
}

export function SpecialWorkDateAssignModal({ assignLogic }: Props) {
  const {
    isOpen,
    setIsOpen,
    selectedTarget,
      ...
      <DialogContent className="!w-[60vw] !max-w-[95vw] max-h-[85vh] flex flex-col bg-gray-900 border-gray-700 text-gray-100">
        <DialogHeader>
          <DialogTitle className="text-white">
            Atur Pegawai Masuk: {selectedTarget?.name}
          </DialogTitle>
          <DialogDescription className="text-gray-400">
            Pilih pegawai yang diwajibkan masuk pada rentang tanggal{" "}
            {selectedTarget?.startDate} s/d {selectedTarget?.endDate}.
          </DialogDescription>
        </DialogHeader>
        ...
        <DialogFooter className="border-t border-gray-800 pt-4 mt-2">
          <span className="text-sm font-medium mr-auto text-emerald-400 mt-2">
            Pegawai Ditugaskan: {selectedUserIds.length} Orang
          </span>
          <Button
            variant="outline"
